...
public class GroupDetailsActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Drawable referenceDrawable = getResources().getDrawable(R.drawable.im_smiley_add_btn);
		...
		adapter = new GridAdapter(this, R.layout.im_grid, group.getMembers());
	...
	private class GridAdapter extends ArrayAdapter<String> {
		...
		public View getView(final int position, View convertView, final ViewGroup parent) {
			...
			if (position == getCount() - 1) {
				...
				if (!group.getOwner().equals(PaopaoOfficialDeliverymanApplication.getInstance().getUserName())) {
				...
				//如果不是创建者或者没有相应权限
				if (!group.isAllowInvites() && !group.getOwner().equals(PaopaoOfficialDeliverymanApplication.getInstance().getUserName())) {
					// if current user is not group admin, hide add/remove btn
					convertView.setVisibility(View.INVISIBLE);
				} else {
					// 正处于删除模式下,隐藏添加按钮
				...
				Drawable avatar = getResources().getDrawable(R.drawable.im_default_avatar);
