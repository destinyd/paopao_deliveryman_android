...
public class ContactAdapter extends ArrayAdapter<IMUser> implements SectionIndexer {
	...
	public ContactAdapter(Context context, int resource, List<IMUser> objects, Sidebar sidebar) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		if (position == 0) {//搜索框
			if(convertView == null){
				convertView = layoutInflater.inflate(R.layout.im_search_bar_with_padding, null);
			...
			if(username.equals(IMConstant.NEW_FRIENDS_USERNAME)){
				...
				avatar.setImageResource(R.drawable.im_new_friends_icon);
			...
			}else if(username.equals(IMConstant.GROUP_USERNAME)){
				//群聊item
				nameTextview.setText(user.getNick());
				avatar.setImageResource(R.drawable.im_groups_icon);
			}else{
				nameTextview.setText(username);
				if(unreadMsgView != null)
					unreadMsgView.setVisibility(View.INVISIBLE);
				avatar.setImageResource(R.drawable.im_default_avatar);
			}
		}
	...
	public IMUser getItem(int position) {
		return position == 0 ? new IMUser() : super.getItem(position - 1);
	}
